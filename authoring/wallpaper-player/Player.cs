...
namespace Ccbcm {
 ...
 public class RemoteWallpaper { public string sha256; public long size; public string mime; }
 ...
 public class Player:Application {
  ...
  [STAThread] public static void Main(string[] args){
   ...
   bool created;using(var mutex=new Mutex(true,"Local\\"+Pipe,out created)){
    ...
    if(arg=="--exit")return;
    try{var app=new Player();app.desktopTest=arg=="--desktop-test";app.test=arg=="--test"||app.desktopTest;app.Startup+=delegate{app.Init(app.test?(args.Length>1?args[1]:""):arg);};app.Run();}catch(Exception e){Log(e.ToString());Forms.MessageBox.Show(e.Message,"CCBCM");}
   }
  }
  ...
  void Init(string arg){
   ...
   tray=new Forms.NotifyIcon{Icon=System.Drawing.Icon.ExtractAssociatedIcon(Process.GetCurrentProcess().MainModule.FileName),Text="CCBCM · 单击打开",ContextMenuStrip=menu,Visible=!test};tray.MouseClick+=delegate(object o,Forms.MouseEventArgs e){if(e.Button==Forms.MouseButtons.Left)ShowSettings();};
   ...
   timer=new DispatcherTimer{Interval=TimeSpan.FromSeconds(1)};timer.Tick+=delegate{ticks++;Update();if(test){if(ticks==2){wallpaper.ClientSize=new System.Drawing.Size(540,960);}if(ticks==3){Log("test_portrait_crop="+media.AppliedCrop);manual=true;Update();Log("test_pause="+paused);}if(ticks==5){wallpaper.ClientSize=new System.Drawing.Size(1000,625);manual=false;Update();Log("test_resume="+!paused);}if(ticks==8){File.WriteAllText(Path.Combine(Dir,"test-result.json"),Json.Serialize(new{opened,openedMs,crop=media.AppliedCrop,hostWidth=media.ClientSize.Width,hostHeight=media.ClientSize.Height,position=media.Position.TotalSeconds,workingSet=Process.GetCurrentProcess().WorkingSet64,desktopAttached=parent!=IntPtr.Zero&&Native.GetParent(hwnd)==parent,paused}));Shutdown();}}};timer.Start();
  ...
  void ShowSettings(){if(settingsWindow!=null){settingsWindow.WindowState=WindowState.Normal;settingsWindow.Activate();return;}settingsWindow=new Window{Title="CCBCM",Icon=System.Windows.Interop.Imaging.CreateBitmapSourceFromHIcon(tray.Icon.Handle,Int32Rect.Empty,System.Windows.Media.Imaging.BitmapSizeOptions.FromEmptyOptions()),Width=530,Height=530,ResizeMode=ResizeMode.CanMinimize,WindowStartupLocation=WindowStartupLocation.CenterScreen,Background=new SolidColorBrush(Color.FromRgb(27,27,34)),Foreground=Brushes.White};var panel=new StackPanel{Margin=new Thickness(28)};var heading=new StackPanel{Orientation=Orientation.Horizontal,Margin=new Thickness(0,0,0,14)};heading.Children.Add(new TextBlock{Text="CCBCM",FontSize=25});var site=new Button{Content="ccbcm.net ↗",Background=Brushes.Transparent,Foreground=Brushes.LightSteelBlue,BorderThickness=new Thickness(0),Margin=new Thickness(15,0,0,0)};site.Click+=delegate{Website();};heading.Children.Add(site);panel.Children.Add(heading);status=new TextBlock{Text="还没有选择壁纸",TextWrapping=TextWrapping.Wrap,Margin=new Thickness(0,0,0,18)};panel.Children.Add(status);
   panel.Children.Add(ActionButton("选择壁纸",Pick));
   panel.Children.Add(ActionButton("检查更新 · 0.4.0",delegate{Updates.Check();}));
   ...
   panel.Children.Add(ActionButton("完全退出 · 恢复原壁纸",delegate{Shutdown();}));settingsWindow.Content=new ScrollViewer{Content=panel,VerticalScrollBarVisibility=ScrollBarVisibility.Auto};settingsWindow.Closed+=delegate{settingsWindow=null;status=null;if(tray!=null){tray.BalloonTipTitle="CCBCM 仍在运行";tray.BalloonTipText="点击桌面的 CCBCM，或右下角托盘图标，可以再次打开。";tray.ShowBalloonTip(4000);}};settingsWindow.Show();Update();}
