 ...
 public class Player:Application {
  ...
  [STAThread] public static void Main(string[] args){
   bool created;using(var mutex=new Mutex(true,"Local\\"+Pipe,out created)){
    ...
    if(arg.Equals("ccbcm-wallpaper://apply/blue",StringComparison.OrdinalIgnoreCase))arg=Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"demo.mp4");else if(arg.StartsWith("ccbcm-wallpaper:",StringComparison.OrdinalIgnoreCase)&&!Regex.IsMatch(arg,@"^ccbcm-wallpaper://apply/[a-f0-9]{64}/?$"))arg="--settings";
  ...
  void Open(string input){if(input.StartsWith("ccbcm-wallpaper://apply/",StringComparison.OrdinalIgnoreCase)){ApplyRemote(input);return;}try{string file=Resolve(input);if((!test||desktopTest)&&!Native.IsWindow(parent))Attach();switchWatch.Restart();opened=false;failed=false;previewUntil=DateTime.UtcNow.AddSeconds(8);media.Close();path=file;resume=TimeSpan.Zero;released=false;manual=false;paused=false;media.Source=new Uri(file);wallpaper.Show();Dispatcher.BeginInvoke(DispatcherPriority.ContextIdle,new Action(FixComposition));if(!test){var r=Forms.Screen.PrimaryScreen.Bounds;Native.SetWindowPos(hwnd,Native.FindWindowEx(parent,IntPtr.Zero,"SHELLDLL_DefView",null),r.Left,r.Top,r.Width,r.Height,0x10);}media.Play();if(!test){config.LastFile=file;Save();}Update();}catch(Exception e){Log(e.Message);if(!test)Forms.MessageBox.Show(e.Message,"CCBCM 壁纸");}}
  ...
  public static string CacheRemote(string id){
   if(!Regex.IsMatch(id,@"^[a-f0-9]{64}$"))throw new Exception("壁纸链接无效。");
   ServicePointManager.SecurityProtocol=SecurityProtocolType.Tls12;
   string root="https://ccbcm.net/api/published-videos/"+id,cache=Path.Combine(Dir,"Cache");RemoteWallpaper meta;
   ...
   if(meta==null||meta.size<32||meta.size>25*1024*1024||!Regex.IsMatch(meta.sha256??"",@"^[a-f0-9]{64}$"))throw new Exception("壁纸信息无效。");
   Directory.CreateDirectory(cache);string file=Path.Combine(cache,id+".mp4");if(File.Exists(file)&&new FileInfo(file).Length==meta.size&&DigestFile(file)==meta.sha256)return file;
   string temp=file+"."+Guid.NewGuid().ToString("N")+".part";
   try{using(var response=Fetch(root+"/content")){if(response.ContentType.Split(';')[0]!="video/mp4")throw new Exception("壁纸格式不支持。");using(var input=response.GetResponseStream())using(var output=File.Create(temp)){byte[] buffer=new byte[65536];long total=0;int n;while((n=input.Read(buffer,0,buffer.Length))>0){total+=n;if(total>meta.size)throw new Exception("壁纸大小不符。");output.Write(buffer,0,n);}if(total!=meta.size)throw new Exception("壁纸未获取完整，请重试。");}}
