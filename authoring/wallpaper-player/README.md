...
新版入口：桌面和开始菜单的「CCBCM 动态壁纸」，或官网的「已安装？打开桌面插件」。程序中提供官网入口、播放进度与暂停原因。添加视频后先试看 8 秒；也可点击「播放试看 30 秒」临时忽略遮挡暂停，电池及锁屏暂停仍生效。关闭设置后可单击托盘重新打开；完全退出则从桌面快捷方式启动。安装程序注册 ccbcm-wallpaper 协议，该协议仅打开管理窗口，不执行网页传入的命令。
