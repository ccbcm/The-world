...
assert.equal((await req('downloads','POST','a'.repeat(64),{id:'waves'})).status,404);
...
assert.equal((await req('downloads','POST',A,{id:'lines'})).status,200);
assert.equal((await req('downloads','DELETE',A,{id:'lines'})).status,200);
...
assert.equal((await req('favorites','POST',A,{id:'lines',user_id:'bob'})).status,200);
assert.equal((await req('favorites','POST',A,{id:'lines'})).status,200);
assert.deepEqual((await (await req('favorites','GET',A)).json()).items.map(x=>x.id),['lines']);
assert.deepEqual((await (await req('favorites','GET',B)).json()).items,[]);
assert.equal((await req('favorites','DELETE',B,{id:'lines'})).status,200);
assert.equal((await (await req('favorites','GET',A)).json()).items.length,1);
assert.equal((await req('favorites','DELETE',A,{id:'lines'},'https://evil.example')).status,403);
...
assert.equal((await req('favorites','DELETE',A,{id:'lines'})).status,200);
...
// New logins are consumers until the owner approves their application.
assert.equal((await (await req('account','GET',A)).json()).creator.status,'none');
assert.equal((await req('videos','POST',A,{})).status,403);
assert.equal((await req('videos/'+'1'.repeat(64),'PATCH',A,{})).status,403);
assert.equal((await req('drafts','POST',A,{title:'x',description:''})).status,403);
assert.equal((await req('creator-application','POST','',{note:'作品'})).status,401);
assert.equal((await req('creator-application','POST',A,{note:'原创动态壁纸'},'https://evil.example')).status,403);
assert.equal((await req('creator-application','POST',A,{note:'原创动态壁纸',status:'approved'})).status,200);
assert.equal((await (await req('account','GET',A)).json()).creator.status,'pending');
assert.equal((await req('creator-application','POST',A,{note:'重复'})).status,409);
...
assert.equal((await req('videos','POST',A,{})).status,403);
assert.equal((await req('creator-application','POST',A,{note:'补充作品链接'})).status,200);
assert.equal((await req('creator-applications/alice','POST',ownerToken,{status:'approved',reason:''})).status,200);
assert.equal((await req('creator-applications/alice','POST',ownerToken,{status:'approved',reason:''})).status,409);
